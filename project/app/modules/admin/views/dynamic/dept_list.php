...
<div class="clearfix"></div>
		  ...
		  <div class="box-body table-responsive no-padding">
                  <table class="table table-hover">
                    <tr>
                     ...
                     <th><?php echo lang('dept_name');?></th>
					<th><?php echo lang('dept_description');?></th>
					
					<th><?php echo lang('dept_status');?></th>
    ...
    if (!empty($result)) {
        foreach ($result as $row) {
            ...
            ?>
            <tr>
		<td><input type="checkbox" name="id_array[]" id="banner" class="id_array" value="<?php echo $row->dept_id;?>" /> </td>
                <td><?php echo $row->dept_name ?></td>
                <td><?php echo truncate_string($row->dept_description,10,25); ?></td>
         
                <?php if ($row->dept_status == 1) { ?>
                    <td><a href="javascript:void(0);" onclick="update_status('<?php echo $row->dept_id; ?>', '<?php echo $param->field;?>', '0');" class="label label-success"><?php echo lang('enabled');?></a></td>
                <?php } else { ?>
                    <td><a href="javascript:void(0);" onclick="update_status('<?php echo $row->dept_id; ?>', '<?php echo $param->field;?>', '1');" class="label label-warning"><?php echo lang('disabled');?></a></td>
                <?php } ?>
                    <!--<td><?php echo get_datepicker_date($row->created_on, 'full');?></td>    -->
                <td>
                    <a class="btn btn-primary btn-sm" href="<?php echo base_url('dept/edit/' . $row->dept_id); ?>">
                        <i class="pe-7s-pen"></i>
                    </a>
					 <a class="btn btn-primary btn-sm" href="<?php echo base_url('dept/view/' . $row->dept_id); ?>">
                        <i class="pe-7s-look"></i>
                    </a>
                    <a class="btn btn-danger btn-sm" onClick="javascript:return confirm('Are you sure you want to Delete?');" href="<?php echo base_url('dept/delete_data/' . $row->dept_id); ?>">
